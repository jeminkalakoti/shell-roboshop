...
###  user Application Installation Steps NodeJS Application ###
...
curl -o /tmp/user.zip https://roboshop-artifacts.s3.amazonaws.com/user-v3.zip &>>$LOG_FILE # Download the application code
VALIDATE $? "Downloading user Application Code"    # Validate the last command
...
unzip /tmp/user.zip &>>$LOG_FILE # Unzip the application code
VALIDATE $? "Unzipping user Application Code"    # Validate the last command 
...
cp $SCRIPT_DIR/user.service /etc/systemd/system/user.service # Copy the service file
...
systemctl enable user &>>$LOG_FILE # Enable the service
VALIDATE $? "Enabling user Service"    # Validate the last command

systemctl start user # Start the service
VALIDATE $? "Starting user Service"    # Validate the last command


systemctl restart user # Restart the service
VALIDATE $? "Restarting user Service"    # Validate the last command
